...
function openEditProfile(){
  ...
  const curCity = ME.city || '';
  const curBio  = (ME.bio != null) ? ME.bio : (bioEl ? bioEl.textContent.trim() : '');
  const esc = s => String(s).replace(/"/g,'&quot;').replace(/</g,'&lt;');
  const overlay = document.createElement('div');
  overlay.className = 'modal-overlay';
  overlay.id = 'edit-profile-overlay';
  overlay.onclick = e => { if (e.target === overlay) closeEditProfile(); };
  overlay.innerHTML = `
    <div class="modal post-modal">
      <div class="modal-header">
        <h3>\u270F\uFE0F ${t('edit_profile_title')}</h3>
        <button class="modal-close" onclick="closeEditProfile()">\u2715</button>
      </div>
      <div class="modal-body">
        <div class="form-group"><label>${t('field_name')}</label>
          <input type="text" id="edit-name" value="${esc(curName)}" /></div>
        <div class="form-group"><label>${t('field_city')}</label>
          <input type="text" id="edit-city" value="${esc(curCity)}" /></div>
        <div class="form-group"><label>${t('field_bio')}</label>
          <textarea id="edit-bio" rows="3">${esc(curBio)}</textarea></div>
        <button class="btn-primary" onclick="saveProfile()">\uD83D\uDCBE ${t('save_changes')}</button>
      </div>
    </div>`;
  document.body.appendChild(overlay);
  document.body.style.overflow = 'hidden';
  setTimeout(()=>{const n=document.getElementById('edit-name');if(n)n.focus();},50);
}

function closeEditProfile(){
  const o = document.getElementById('edit-profile-overlay');
  if (o) o.remove();
  document.body.style.overflow = '';
}
...
function saveProfile(){
  ...
  if (!name){ showToast('\u26A0\uFE0F ' + t('toast_name_required')); return; }
  ME.name = name; ME.city = city; ME.bio = bio;
