...
## R42 - 2026-08-10 — סגירת ה-15 שנשארו
הסבב הקודם סגר את הקריטי והגבוה. כאן נסגר כל השאר, כולל תוכן מומצא ש-R33 טען שסולק.

**מה שמשתמש היה נתקל בו**
- **בעמוד "שמורים" הלייק והתגובה לא הגיבו.** אותה מודעה יכולה להיות על המסך פעמיים — בפיד ובשמורים — עם אותם מזהים, ו-`querySelector` החזיר את העותק המוסתר. עכשיו כל עותק מתעדכן.
- **מונה התגובות קפץ למספר שגוי** כשמישהו הגיב בזמן אמת: הוא נקבע לפי אורך מערך שמאז R37 ריק עד שפותחים את השרשור. מודעה עם 12 תגובות הפכה ל-1.
- **תוצאות חיפוש נשארו תקועות בפיד** לתמיד, כולל בספירות ובלוח — ובלי מצב הלייק שלך, כך שלחיצה על הלב נכשלה בשקט.
- **העימוד יצר כפילויות:** מודעה שהגיעה בזמן אמת ולא הוצגה לא קידמה את החלון, אז העמוד הבא שלף מחדש שורות שכבר היו.
- **הגדרת התראות הסתירה תוכן.** תיבת סימון בשם "התראות רק מהאזור שלי" קבעה אם מודעה נכנסת לפיד בכלל — ולא באופן עקבי: הטעינה הראשונה לא סוננה, רק מה שהגיע חי.

**ומה שעדיין היה מומצא**
- **"גלה" הכריז על 34 מודעות בתל אביב ו-21 בחיפה** — מספרים קשיחים ב-HTML מול מסד ריק. נספרים באמת עכשיו.
- **הפרופיל הציג 89 עוקבים ו-43 נעקבים.** אין פיצ'ר עוקבים; המספרים הוסרו במקום להמציא.
- הקומפוזר הציג פרצוף של אדם אחר למשתמש מחובר.

**בצד השרת**
- **הגבלת הקצב התאפסה במחיקה** — היא ספרה שורות קיימות, אז מחיקה החזירה מכסה. עכשיו נספר יומן שאי אפשר לכתוב אליו או למחוק ממנו דרך ה-API.
- **דיווח היה בלתי הפיך** ולא היה למי לערער. עכשיו אפשר לבטל, והספירה וההסתרה מחושבות מחדש.
- **דיווחים לא הוגבלו בקצב** — שלושה חשבונות זולים הסתירו מודעה בחינם.
- **בעל מודעה לא יכול היה להסיר תגובה** על המודעה שלו.
- **האחסון קיבל כל סוג קובץ** בשורש הדלי, כולל HTML על הדומיין שלך. עכשיו תמונות ווידאו בלבד, כל חשבון תחת תיקייה משלו.
- **CSP** ועוד ארבע כותרות אבטחה — שכבת ההגנה השנייה שחסרה.
- **טעינה ראשונה בלי timeout** השאירה שלדים לנצח מול שרת שנתקע.

**ובדרך שברתי ותיקנתי שוב:** הטריגר החדש של הגבלת הקצב קרא `user_id`, ובטבלת הדיווחים העמודה היא `reporter_id` — מה שהשבית דיווח לחלוטין. נתפס בבדיקה.

אומת: 22/22 בזרימה, 16/16 במודרציה, 7/7 בסבב הזה.

## R41 - 2026-08-10 — הפיד הוריד יותר מדי
נמדד מול 300 מודעות ב-60 בקשות במקביל: select=* לקח 896ms ו-39KB, ואותה שאילתה בלי שדה התיאור לקחה 209ms ו-9KB. **התיאור הוא שלושה רבעים ממשקל הפיד, בשביל כרטיס שמציג ממנו שתי שורות.**
- עמודת summary שנוצרת אוטומטית במסד (200 תווים ראשונים) — בלי לוגיקה באפליקציה שצריך לתחזק.
- הפיד, החיפוש, העימוד וההשלמה קוראים רשימת עמודות מפורשת עם summary. הטקסט המלא נטען רק כשפותחים מודעה.
- אחרי: 289ms ו-26KB — פי שלושה מהיר יותר על אותה שאילתה.

**ומה שנמדד ב-200 משתמשים על פיד עם 300 מודעות:** חציון 183ms, אבל p99 של 8 שניות. אפס שגיאות ב-2,268 בקשות, והנתונים יצאו מדויקים. זה גבול המכונה בתוכנית החינמית, לא של הקוד — הפעולות הקלות (פרופיל, לייק, תגובה) נשארו ב-85ms גם תחת העומס הזה.

## R40 - 2026-08-10 — מה ששני סוקרים חיצוניים מצאו
שני סוכנים סקרו את הקוד באופן עצמאי לפני הפריסה. הם מצאו דברים אמיתיים, כולל שלושה שדיווחתי כמושלמים ולא עבדו.

**קריטי**
- **פרסום מודעה גרם לה להיעלם.** `liveFiltered()` מסתיר כל פוסט בלי `dbId` כשיש שרת, ו-`backendPublish` קבע את ה-`dbId` אבל לא רינדר מחדש. המפרסם ראה "פורסם!" ואז מסך ריק; רק רענון הראה את המודעה.
- **XSS דרך מזהה השיחה.** `cv.id` נבנה מ-`room` — טקסט חופשי שהשולח בוחר — והוזרק גולמי ל-`id=` ול-`onclick=`. מחרוזת מתאימה סוגרת את המאפיין ומריצה קוד בדפדפן הנמען. המזהים עכשיו מונה מקומי, וסרגל הקלט מחווט במאזינים.
- **`esc()` לא מגן בתוך `onclick`.** הוא הופך גרש ל-`&#39;`, והפרסר מפענח ישויות לפני שה-JS מתקמפל — כלומר שם תצוגה כמו `');alert(1)//` רץ. אין יותר נתוני משתמש בתוך markup.

**גבוה**
- **החסימה מעולם לא עבדה.** הועבר לה `post.user.id` שהוא `db-<מזהה דפדפן>`, בעוד העמודה היא uuid. כל ניסיון נכשל בשגיאת טיפוס. עכשיו לפי `ownerUid`.
- **ביטול עריכה הפך את הפוסט הבא לדריסה** — `editingPost` נוקה רק בשמירה.
- **אין חיפוש ואין מסננים בטלפון** — שניהם `display:none` מתחת ל-640 בלי תחליף, כך שחיפוש המסד מ-R37 והסינון לפי הערים שלך מ-R29 לא היו נגישים במכשיר העיקרי.
- **כשלונות כתיבה דווחו כהצלחה** — פרסום, לייק, תגובה והודעה נכשלו בשקט. עכשיו מתגלגלים אחורה עם שגיאה מובנת.
- **הבעלים יכול היה לבטל מודרציה על עצמו** — `PATCH` אחד החזיר `hidden` ל-false, איפס דיווחים וזייף מוני לייקים. טריגר נועל עכשיו את העמודות המוגנות, ולמנהל יש הרשאת עדכון אמיתית.
- **`created_at` היה ניתן לכתיבה** — תאריך עתידי נעץ מודעה בראש הפיד לתמיד.
- **בלי מגבלות אורך** — תיאור של 10MB הוא הורדה של 10MB לכל מבקר, כי הפיד מושך שורות שלמות.
- **`blocked_between` נחשף כ-RPC** וענה על כל זוג משתמשים — אפשר היה למפות את גרף החסימות. הוחלף ב-`blocked_with` שיודע לענות רק עליך.

**ובדרך שברתי ותיקנתי:** המגן על העמודות חסם גם את הטריגרים הפנימיים שמעדכנים מונים ודיווחים, אז ההסתרה האוטומטית הפסיקה לעבוד. הכותבים הפנימיים מרימים דגל ברמת הטרנזקציה שהמגן מכבד; ללקוח אין דרך להרים אותו.

אומת: 22/22 בזרימה המלאה, 16/16 במודרציה, 8/8 בהקשחה.

## R39 - 2026-08-10 — הגנות לקהל אמיתי
הכל נאכף בבסיס הנתונים. בדיקה שקיימת רק בדפדפן היא הצעה מנומסת — מי שפותח קונסולה עוקף אותה בשורה אחת. אומת ע"י פנייה ישירה ל-API, בעקיפת הממשק: 16/16.
- **דיווח על מודעה** — חמש סיבות, דיווח אחד לאדם. שלושה מדווחים שונים מסתירים את המודעה אוטומטית עד בדיקה. הבעלים ומנהל עדיין רואים אותה; אף אחד אחר לא.
- **חסימת משתמש** — המודעות והתגובות שלו נעלמות, ואי אפשר להתכתב באף כיוון. נאכף במדיניות, לא בסינון בדפדפן. החסימה פרטית והפיכה.
- **הגבלת קצב** — 10 מודעות, 40 תגובות ו-150 הודעות לשעה למשתמש. טריגר שרץ לפני ההכנסה ודוחה. נבדק: הצפה של 14 מודעות נעצרה אחרי 10.
- **מנהלים** — טבלת admins ופונקציית is_admin(), שמאפשרות לראות מודעות מוסתרות ולמחוק כל מודעה.
- שגיאות שרת מתורגמות למשהו שאפשר לפעול לפיו במקום הודעת Postgres גולמית.
